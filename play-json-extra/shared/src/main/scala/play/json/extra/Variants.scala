...
object Variants {
  ...
  /**
    * @tparam A The base type of a final case class hierarchy.
  ...
  /**
    ...
    * @tparam A Base type of final case class hierarchy.
    * @return A [[play.api.libs.json.Format]] for the type hierarchy of `A`.
    */
  def format[A](discriminator: Format[String]): Format[A] = macro Impl
    .formatDiscriminator[A]
  ...
  /**
    ...
    */
  def reads[A](discriminator: Reads[String]): Reads[A] = macro Impl
    .readsDiscriminator[A]
  ...
  /**
    ...
    */
  def writes[A](discriminator: Writes[String]): Writes[A] = macro Impl
    .writesDiscriminator[A]
  ...
  private object Impl {
    ...
    def formatDiscriminator[A: c.WeakTypeTag](c: Context)(
        discriminator: c.Expr[Format[String]]): c.Expr[Format[A]] = {
      import c.universe._
      ...
      c.Expr[Format[A]](
          q"play.api.libs.json.Format[$baseClass]($reads, $writes)")
    }
    ...
    def writesDiscriminator[A: c.WeakTypeTag](c: Context)(
        discriminator: c.Expr[Writes[String]]): c.Expr[Writes[A]] = {
      val (baseClass, variants) = baseAndVariants[A](c)
     ...
     */
    def baseAndVariants[A: c.WeakTypeTag](
        c: Context): (c.universe.ClassSymbol, Set[c.universe.ClassSymbol]) = {
      import c.universe._
      ...
      for (variant <- variants
           if !(variant.isCaseClass || variant.isModuleClass)) {
        c.abort(c.enclosingPosition,
                s"$variant is not a final case class nor a case object")
      }
      baseClass -> variants
    }

    def writesTree(c: Context)(
        baseClass: c.universe.ClassSymbol,
        variants: Set[c.universe.ClassSymbol],
        discriminator: c.Expr[Writes[String]]): c.Tree = {
      import c.universe._
    ...
    def readsTree(c: Context)(baseClass: c.universe.ClassSymbol,
                              variants: Set[c.universe.ClassSymbol],
                              discriminator: c.Expr[Reads[String]]): c.Tree = {
      import c.universe._
      val readsCases = for (variant <- variants) yield {
        if (!variant.isModuleClass) {
          ...
          cq"""${variant.name.decodedName.toString} => play.api.libs.json.JsSuccess(${TermName(
              variant.name.decodedName.toString)})"""
        }
      }
      q"""
