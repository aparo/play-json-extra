...
package object internals {
  ...
  object CaseClass {
    def checkCaseClassMacro[T: c.WeakTypeTag](c: blackbox.Context) = {
      ...
      q"new _root_.play.json.extra.internals.CaseClass[$T]"
    }

    /**
    fails compilation if T is not a final case class
  ...
  object SingletonObject {
    def checkSingletonObjectMacro[T: c.WeakTypeTag](c: blackbox.Context) = {
      import c.universe._
      val T = c.weakTypeOf[T]
      if (!T.typeSymbol.isClass || !T.typeSymbol.asClass.isModuleClass)
        c.error(c.enclosingPosition, "")
      q"new _root_.play.json.extra.internals.SingletonObject[$T]"
    }
    ...
    meaning this can be used as an implicit to check
      */
    implicit def checkSingletonObject[T]: SingletonObject[T] = macro checkSingletonObjectMacro[
        T]
  }

  import scala.collection._
  import scala.collection.generic.CanBuildFrom
  private[json] implicit class TraversableLikeExtensions[A, Repr](
      val coll: TraversableLike[A, Repr])
      extends AnyVal {
    ...
    def distinctWith[That](equivalent: (A, A) => Boolean)(
...
TRIGGERED BY: could not find implicit value for parameter helper: play.json.extra.OptionValidationDispatcher[${T}]
TO SOLVE THIS
...
""")
final class OptionValidationDispatcher[T] private[json] (
    val validate: JsLookupResult => JsResult[T])
    extends AnyVal
...
object OptionValidationDispatcher {
  ...
  def dispatch[T](reads: Reads[T])(
      disambiguate: AnyRef = null): OptionValidationDispatcher[T] = {
  ...
  implicit def dispatchOption[T: Reads]
    : OptionValidationDispatcher[Option[T]] = {
...
object `package` {
  implicit class JsLookupResultExtensions(res: JsLookupResult) {
    ...
    def validateAuto[T](
        implicit helper: OptionValidationDispatcher[T]): JsResult[T] =
      helper.validate(res)
  }
...
private[extra] class Macros(val c: blackbox.Context) {
  ...
  private def knownTransitiveSubclasses(sym: ClassSymbol): Seq[ClassSymbol] = {
    sym +: (if (sym.isModuleClass) {
              ...
              sym.knownDirectSubclasses.flatMap(s =>
                    knownTransitiveSubclasses(s.asClass))
            }).toSeq
  }
  ...
  private def caseClassFieldsTypes(
      tpe: Type): ListMap[String, (String, Type)] = {
    val paramLists = primaryConstructor(tpe).paramLists
    val params = paramLists.head

    if (paramLists.size > 1)
      c.error(c.enclosingPosition,
              s"Only one parameter list classes are supported. Found: $tpe")
        ...
        field.annotations.map { annotation =>
          annotation.tree match {
            case Apply(Select(New(tpe), _), List(Literal(Constant(unique)))) =>
              if (tpe.toString().toLowerCase.endsWith(".key"))
                attrName = unique.toString
            case extra =>
          }
        }
      }
      (field.name.toTermName.decodedName.toString,
       (attrName -> field.infoIn(tpe)))
    }: _*)
  }
  private def caseClassFieldsDefaults(
      tpe: Type): ListMap[String, Option[Tree]] = {
  ...
  def formatAutoInternal(T: Type): Tree = {
    ...
    def defaultFormatter =
      if (T <:< typeOf[Option[_]]) {
        ...
        val ArgType = caseClassFieldsTypes(T).head._2._2
        ...
        val fieldFormatters = T.typeSymbol.asClass.knownDirectSubclasses.map {
          t =>
            val name = TermName(c.freshName)
            q"implicit def $name = Jsonx.formatAuto[$t]"
        }
  ...
  def formatInline[T: c.WeakTypeTag]: Tree = {
    ...
    if (fields.size != 1)
      c.error(c.enclosingPosition,
              s"class with exactly one argument required, but found: $T")
    val (field, (attrName, tpe)) = fields.head
    //TODO fix attrname in inline
    q"""
      {
        import $pjson._
        import $pkg._
  ...
  def formatCaseClassUseDefaults[T: c.WeakTypeTag](ev: Tree): Tree =
    formatCaseClassInternal[T](ev, true)

  def formatCaseClass[T: c.WeakTypeTag](ev: Tree): Tree =
    formatCaseClassInternal[T](ev, false)

  private def formatCaseClassInternal[T: c.WeakTypeTag](
      ev: Tree,
      useDefaults: Boolean): Tree = {
    val T = c.weakTypeOf[T]
    if (!isCaseClass(T))
      c.error(c.enclosingPosition, s"not a final case class: $T")
    val defaults = caseClassFieldsDefaults(T)
    def orDefault(t: Tree, name: String) = {
      val default = defaults.get(name).flatten
      default
        .filter(_ => useDefaults)
        .map(d => q"$t orElse JsSuccess($d)")
        .getOrElse(t)
    }
    val (results, mkResults) = caseClassFieldsTypes(T).map {
      case (k, (a, t)) =>
        ...
        (name,
         q"""val $name: JsResult[$t] = {
            val bpath = $path
            val path = (JsPath() \ $a)
            val resolved = path.asSingleJsResult(json)
            val result = if(bpath.isInstanceOf[JsDefined]) ${result} else ${orDefault(
             result,
             k)}
    ...
    val jsonFields = caseClassFieldsTypes(T).map {
      case (k, (a, t)) =>
        q"""${Constant(a)} -> Json.toJson[$t](obj.${TermName(k)})(implicitly[Writes[$t]])"""
    }
  ...
  private def verifyKnownDirectSubclassesPostTyper(_T: Type,
                                                   macroCall: String): Tree = {
    ...
    def checkSubsPostTyper =
      if (subs != T.knownDirectSubclasses)
        c.error(
...
""")
    ...
    val checkSubsPostTyperTypTree =
      new global.TypeTreeWithDeferredRefCheck()(() => {
        checkSubsPostTyper; global.TypeTree(global.NoType)
      }).asInstanceOf[TypTree]
    q"type VerifyKnownDirectSubclassesPostTyper = $checkSubsPostTyperTypTree"
  }

  private def assertClass[T: c.WeakTypeTag](
      msg: String = s"required class or trait") {
  ...
  private def assertSealedAbstract[T: c.WeakTypeTag] {
    ...
    if (!T.isSealed || !T.isAbstract) {
      ...
      c.error(
          c.enclosingPosition,
          s"required sealed trait or sealed abstract class, found $modifiers ${T.fullName}")
    }
  }

  def formatSingletonImplicit[T: c.WeakTypeTag](encodeSingleton: Tree,
                                                ev: Tree): Tree =
    formatSingleton[T](encodeSingleton)
  ...
  def formatSealedWithFallback[T: c.WeakTypeTag, Fallback <: T: c.WeakTypeTag]
    : Tree =
    formatSealedInternal[T](Some(c.weakTypeOf[Fallback].typeSymbol.asType))
  def formatSealedInternal[T: c.WeakTypeTag](
      fallback: Option[TypeSymbol]): Tree = {
...
This can be caused by https://issues.scala-lang.org/browse/SI-7046 which can only be avoided by manually moving the call.
    ...
    val writes = subs.map { sym =>
      cq"""obj: $sym => Json.toJson[$sym](obj)(implicitly[Format[$sym]])"""
    }

    val reads = subs
    // don't include fallback
      .filterNot(t =>
            fallback.map(_.toType =:= t.asType.toType).getOrElse(false))
      .map { sym =>
        q"""json.validateAuto[$sym]"""
      }
      .reduce((l, r) => q"$l orElse $r")

    // add fallback last
    val readsWithFallback = fallback.map(f =>
            q"$reads orElse json.validateAuto[$f]") getOrElse reads
    ...
    val t = q"""
      {
        ...
        new Format[$T]{
          ...
          def writes(obj: $T) = {
            obj match {
              case ..$writes
              case _ => throw new Exception("formatSealed found unexpected object of type "+${Literal(
        Constant(T.toString))}+s": $${obj.getClass}$$obj")
            }
          }
        }
      }
...
object SingletonEncoder {
  ...
  implicit def simpleName =
    SingletonEncoder(cls => JsString(decodeName(cls.getSimpleName)))
  implicit def simpleNameLowerCase =
    SingletonEncoder(
        cls => JsString(camel2underscore(decodeName(cls.getSimpleName))))
  implicit def simpleNameUpperCase =
    SingletonEncoder(
        cls =>
          JsString(
              camel2underscore(decodeName(cls.getSimpleName)).toUpperCase))
}

object Jsonx {

  /**
  Generates a PlayJson Format[T] for a final case class T with any number of fields (>22 included)
    */
  def formatCaseClass[T](implicit ev: CaseClass[T]): Format[T] = macro Macros
    .formatCaseClass[T]
  ...
  Uses default values when fields are not found
    */
  def formatCaseClassUseDefaults[T](implicit ev: CaseClass[T]): Format[T] = macro Macros
    .formatCaseClassUseDefaults[T]
  ...
  in case of ambiguities.
    */
  def formatSealed[T]: Format[T] = macro Macros.formatSealed[T]

  /**
  Generates a PlayJson Format[T] for a sealed trait that dispatches to Writes of it's concrete subclasses.
  Uses provided type Fallback as the last resort. Fallback needs to be a subtype of T
  and ideally: final case class Fallback(json: JsValue) extend T
  and using formatInline[Fallback] as the serializer
  CAREFUL: It uses orElse for Reads in an unspecified order, which can produce wrong results
  in case of ambiguities.
    */
  def formatSealedWithFallback[T, Fallback <: T]: Format[T] = macro Macros
    .formatSealedWithFallback[T, Fallback]
