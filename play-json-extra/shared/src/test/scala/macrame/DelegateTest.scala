...
class DelegateTest extends FunSuite {
   ...
   test("Delegating should work in case classes.") {
      assertCompiles("""
      final case class Admin(@delegate underlying : User, privileges : List[String])
   ...
   final case class CaseOne(foo : String, bar : Int, baz : Long)
   test("Delegated parameters should not clobber copy methods.") {
      final case class CaseTwo(@delegate one : CaseOne, alpha : Boolean)
