...
import org.joda.time.chrono.{ChronoLocalDate, Era, IsoChronology}
...
class LocalDate protected[time](f: LocalDateF) extends Wraps(f) with ChronoLocalDate {
  def atStartOfDay(): DateTime = f.atStartOfDay()
  def atStartOfDay(zone: ZoneId): DateTime = f.atStartOfDay(zone)
  def atTime(hour: Int, minute: Int): DateTime = f.atTime(hour, minute)
  def atTime(hour: Int, minute: Int, second: Int): DateTime = f.atTime(hour, minute, second)
  def atTime(hour: Int, minute: Int, second: Int, nanoOfSecond: Int): DateTime = f.atTime(hour, minute, second, nanoOfSecond)
  def atTime(time: LocalTime): DateTime = f.atTime(time)
