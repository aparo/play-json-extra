...
import org.joda.time.temporal.{Temporal, TemporalAccessor, TemporalAdjuster, TemporalAmount, TemporalField, TemporalUnit}
...
class LocalTime protected[time](f: LocalTimeF) extends Wraps(f) with Comparable[LocalTime]
  ...
  def atDate(date: LocalDate): DateTime = f.atDate(date)
